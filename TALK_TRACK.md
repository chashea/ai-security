...
> and the new Agent 365 control plane — combine to secure AI workloads,
> agents, and the data flowing through them.
>
...
| **Purview** | "What data is going in and coming out?" DSPM for AI, DLP for Copilot and custom agents, sensitivity labels that survive grounding, insider risk. |
...
| **Agent 365** | "The new control plane for agents themselves." Registry, access control, topology visualization, agent-specific threat detection. |
...
> ingests through a grounding connector. Four things happen, in seconds:
>
> 1. **Azure AI Content Safety** sees the injection pattern at the model
>    boundary — that's prevention.
> 2. **Defender for Cloud** fires an `AI.PromptInjectionAttempt` alert
>    with the payload hash and a MITRE mapping — that's detection.
> 3. **Defender XDR** joins that alert to sign-in logs and prior
>    reconnaissance from the same IP — that's correlation.
> 4. **Entra Continuous Access Evaluation** revokes the attacker's tokens
>    and forces phishing-resistant MFA — that's response.
>
...
- **Phase 1 — Weeks 1–2: Discover & Assess.** Turn on Purview DSPM for AI,
  ...
  permissions with CIEM, inventory every agent in Agent 365. *Outcome:
  you have a baseline.*
- **Phase 2 — Weeks 3–4: Classify & Protect.** Deploy DLP for Copilot,
  Conditional Access policies for AI apps, sensitivity labels on
  grounding data, scope agent access. *Outcome: the easy wins are live.*
...
- **Phase 4 — Ongoing: Harden & Govern.** CIEM right-sizing, attack-path
  remediation, insider risk tuning, agent topology reviews, workload
  identity federation. *Outcome: this becomes operational, not a project.*
...
> "The fastest concrete win is usually Agent 365 + Purview DSPM for AI.
> Within a week you'll have an inventory of every agent in your tenant
> and a map of what data each one can reach. That alone usually surfaces
> 2–3 agents that need immediate scoping."
...
| *"We already use \<third-party DLP / CSPM / SIEM\>."* | Great — the signal model still applies. Purview, Defender, and Entra all export to other SIEM/SOAR via Graph Security API and Sentinel connectors. The value is the AI-specific signal, not lock-in. |
...
| *"Our agents are built in \<non-Microsoft framework\>."* | Agent 365 is designed around open standards (MCP, A2A) so non-Microsoft agents can be registered and governed. The data plane controls (Purview DLP, DfC, Entra CA) apply regardless of the agent framework. |
